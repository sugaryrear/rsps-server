...
public class UmbraCombat extends CommonCombatMethod {
    ...
    private void doMagic(Mob mob, Mob target) {
        ...
        Chain.bound(null).runFn(2, () -> {
            new Projectile(new Tile(mob.tile().x + -1, mob.tile().y + 1), lightning_one, 0, 378, 10 * tileDist, delay, 70, 45, 0).sendProjectile();
            new Projectile(new Tile(mob.tile().x + -1, mob.tile().y + 1), lightning_two, 0, 378, 10 * tileDist, delay, 70, 45, 0).sendProjectile();
            new Projectile(new Tile(mob.tile().x + -1, mob.tile().y + 1), lightning_three, 0, 378, 10 * tileDist, delay, 70, 45, 0).sendProjectile();

            World.getWorld().tileGraphic(383, lightning_one,0, 10 * tileDist);
            World.getWorld().tileGraphic(383, lightning_two, 0, 10 * tileDist);
            World.getWorld().tileGraphic(383, lightning_three, 0, 10 * tileDist);
        }).then(3, () -> {
