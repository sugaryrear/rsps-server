...
public class GlaciesCombat extends CommonCombatMethod {
    ...
    private void attackregular (Mob mob, Mob target) {
        mob.animate(1978);
        new Projectile(mob, target, 360, 45, 65, 55, 35, 0).sendProjectile();
    ...
    private void doMagic(Mob mob, Mob target) {
        ...
        Chain.bound(null).runFn(2, () -> {
            new Projectile(new Tile(mob.tile().x + -1, mob.tile().y + 1), lightning_one, 0, 360, 10 * tileDist, delay, 70, 45, 0).sendProjectile();
            new Projectile(new Tile(mob.tile().x + -1, mob.tile().y + 1), lightning_two, 0, 360, 10 * tileDist, delay, 70, 45, 0).sendProjectile();
            new Projectile(new Tile(mob.tile().x + -1, mob.tile().y + 1), lightning_three, 0, 360, 10 * tileDist, delay, 70, 45, 0).sendProjectile();

            World.getWorld().tileGraphic(363, lightning_one,0, 10 * tileDist);
            World.getWorld().tileGraphic(363, lightning_two, 0, 10 * tileDist);
            World.getWorld().tileGraphic(363, lightning_three, 0, 10 * tileDist);
        }).then(3, () -> {
