...
public class TopXP implements Serializable {

    private static final long serialVersionUID = -3355621107999346623L;
     ...
     */
    private static final Comparator<Map.Entry<String, Long>> HIGHEST_VALUE = Map.Entry.comparingByValue();
     ...
     */
    public void update(String player, Long amount) {
        if (!entries.containsKey(player)) {
            entries.put(player, amount);
        } else {
            long oldValue = entries.get(player);
            entries.replace(player, oldValue, oldValue + amount);
        }
     //   entries.replace(player, amount);
     ...
     * @return the winner
     */
    public ArrayList<Map.Entry<String, Long>> getSortedResults() {//the actual number of winners
        List<Map.Entry<String, Long>> list = new ArrayList<>(entries.entrySet());
    ...
    public ArrayList<Map.Entry<String, Long>> getSortedResultsTopPkersOnline() {//the number to show on the scoreboard
        List<Map.Entry<String, Long>> list = new ArrayList<>(entries.entrySet());
        list.sort(HIGHEST_VALUE);
        Collections.reverse(list);
     ...
     */
    public void setWinners(Map<String, Long> winners) {
