...
public class PlayersOnlineDatabaseTransaction extends VoidDatabaseTransaction {
    private static final Logger logger = LogManager.getLogger(PlayersOnlineDatabaseTransaction.class);
    int id;
    int amount;

    public PlayersOnlineDatabaseTransaction(int id, int amount) {
        this.id = id;
        this.amount = amount;
    }
    ...
    public void executeVoid(Connection connection) throws SQLException {
        if(!GameConstants.production)
            return;
        try (NamedPreparedStatement statement = prepareStatement(connection,"UPDATE onlineusers SET amount = :amount WHERE id = :id")) {
